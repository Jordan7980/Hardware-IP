...
XFir_filter_Config *XFir_filter_LookupConfig(UINTPTR BaseAddress) {
	...
	for (Index = (u32)0x0; XFir_filter_ConfigTable[Index].Name != NULL; Index++) {
		if (!BaseAddress || XFir_filter_ConfigTable[Index].Ctrl_BaseAddress == BaseAddress) {
...
int XFir_filter_Initialize(XFir_filter *InstancePtr, UINTPTR BaseAddress) {
	...
	ConfigPtr = XFir_filter_LookupConfig(BaseAddress);
	if (ConfigPtr == NULL) {
